...
```sh
git pull
docker build -f deploy/Containerfile -t scrubberd:0.7.0 .
docker save -o dist/scrubberd-0.7.0.tar scrubberd:0.7.0
```

On the isolated side:

```sh
docker load -i scrubberd-0.7.0.tar
```
