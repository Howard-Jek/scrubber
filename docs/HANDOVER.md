...
```sh
git pull
docker build -f deploy/Containerfile -t scrubberd:0.8.4 .
docker save -o dist/scrubberd-0.8.4.tar scrubberd:0.8.4
```

On the isolated side:

```sh
docker load -i scrubberd-0.8.4.tar
```
...
docker build -f deploy/Containerfile \
  ...
  -t <artifactory>/docker-local/scrubberd:0.8.4 .
```
