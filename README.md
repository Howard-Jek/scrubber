...
| `file-timeout` | One **file** was still being scrubbed when its `FILE_SCRUB_TIMEOUT` ran out. Only that file is passed through; the rest of the archive is still scrubbed. Cost follows match *density*, not size, so this catches the dense file that `leaf-cap` cannot see. **Fix:** raise `FILE_SCRUB_TIMEOUT`, or give the pod more CPU — the walk is single-threaded. Off by default. |
| `git-pack` | A **git packfile** (`.git/objects/pack/*.pack`) — the repository's whole history, including content deleted from the working tree. Every object is inflated and scanned and the report **names the object IDs** carrying matches, but the pack is passed through unchanged: object IDs are the SHA-1 of their own content, so redacting a byte leaves a repository git cannot open. Objects stored as deltas are resolved against their bases first, so a secret merely *carried forward* by a later commit is found rather than only the commit that introduced it. A pack carrying matches makes the run `incomplete-risky` and diverts it to `review/`. **Fix:** remove `.git` from the bundle, or rewrite history with `git filter-repo`. |
